"""
v21: target-exposure action.

Hypothesis: v18's action is share-delta — `raw_action ∈ [-1,1]` × hmax = number
of shares to buy or sell. That couples credit-assignment to the price level: a
+0.5 action means "buy hmax/2 shares" regardless of how many shares we already
hold or what the price is. The policy has to learn three things at once: when
to be long, how big to be, and how to get there from the current position.

Target-exposure action decouples the first two from the third. The agent
outputs `target ∈ [0, 1]` (mapped from the raw `[-1, 1]` PPO action via
(a + 1) / 2), interpreted as "fraction of total capital that should be held in
the stock". The env computes the share-delta needed to hit that target. Now
the agent learns ONE thing — what exposure level matches the regime — and
the env handles the bookkeeping. This is canonical in portfolio-RL papers
(Jiang et al, Liang et al).

Single-variable change vs v18: same reward (v12 log-return + DD penalty), same
LSTM, same hyperparameters, same splits. Only `_process_action` is rewritten
and `step()` no longer needs the `rescaled = action_shares / hmax` round-trip
(we just compute the integer share-delta directly and feed FinRL the rescaled
form it expects).

Expected effect: smoother training curves on high-priced stocks (RELIANCE,
TCS) where price-level coupling was worst. Action variance should fall — agent
is no longer forced to constantly re-emit small share-deltas; it just emits a
target. Diagnose via std-trajectory in TB logs (should converge faster) and
trade count (may go down — fewer "fix-up" trades to maintain a target).

Why this beats v18 if successful: cleaner credit assignment. Same return-target
exposure stays valid as price changes; share-delta does not.
"""
...
class IntegerTradingEnv(StockTradingEnv):
    ...
    def _process_action(self, raw_action):
        ...
        raw_action ∈ [-1, 1] is mapped to target ∈ [0, 1] via (a + 1) / 2 and
        interpreted as the FRACTION of total portfolio value that should be
        held in this stock. The env computes the integer share-delta required
        to hit that target given the current cash, holdings, and price.

        Why this differs from v18's share-delta action:
          - v18: action × hmax = number of shares to buy/sell — agent must
            re-discover sizing every step as price moves.
          - v21: action = target exposure — agent emits "how much risk should
            I have right now", env handles the conversion. Same target stays
            valid as price changes.

        Returned integer share-delta is still clamped by budget (for buys) and
        current holdings (for sells), and capped at ±hmax per step so a single
        bar can't completely repriced the portfolio.
        """
        raw_action = np.asarray(raw_action, dtype=np.float64).reshape(-1)
        # Map [-1, 1] -> [0, 1] target-exposure fraction.
        target_frac = np.clip((raw_action + 1.0) * 0.5, 0.0, 1.0)
        ...
        # Total portfolio value at the start of this step (post-price-update by FinRL).
        prices_safe = np.where(prices < 1e-6, 0.01, prices)
        portfolio_value = cash + float(np.sum(shares * prices_safe))

        action_shares = np.zeros(self.stock_dim, dtype=int)
        ...
        for i in range(self.stock_dim):
            ...
            # Desired notional in stock i = target_frac[i] * portfolio_value.
            # Desired integer-share count from that notional.
            desired_notional = target_frac[i] * portfolio_value
            desired_shares = int(desired_notional // price) if price > 1e-6 else 0
            # Delta from current holdings to the desired level.
            delta = desired_shares - int(shares[i])
            # Cap per-step movement at ±hmax so a single bar can't liquidate
            # or fully load the book — keeps the action distribution bounded
            # the same way v18 did.
            delta = int(np.clip(delta, -self.hmax, self.hmax))

            if delta > 0:  # Buy — clamp to budget after costs
                denominator = price * (1 + self.buy_cost_pct[i])
                max_affordable = int(cash // denominator) if denominator > 1e-6 else 0
                delta = int(min(delta, max_affordable))
            elif delta < 0:  # Sell — can't sell more than we hold
                delta = int(max(delta, -int(shares[i])))

            action_shares[i] = delta
