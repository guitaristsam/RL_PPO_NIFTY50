...
class IntegerTradingEnv(StockTradingEnv):
    ...
    def __init__(self, **kwargs):
        ...
        # v11 reward state — back to v9's log-return reward, plus a MILD
        # drawdown penalty. v10 used Moody-Saffell DSR which produced reward
        # magnitudes large enough to blow up value_loss to 85 and collapse
        # clip_fraction to 1e-4 (no policy learning).
        self._equity_peak = float(self.initial_amount)
        self._dd_threshold = 0.10       # only penalize drawdowns deeper than this
        self._dd_lambda    = 1.0        # was 5.0 in v10 — too aggressive

    def _compute_reward_v11(self, log_return, total_asset):
        """v11 reward = log-return × 100 (v9's reward, wider clip) - mild DD penalty.

        Why v11 reverted v10's DSR:
          - v10 RELIANCE: value_loss exploded to 85 (vs 0.3-3.4 in v9 batch),
            clip_fraction collapsed to 1e-4 — PPO essentially stopped updating
            the policy. The DSR formula has a `(B - A²)^(3/2)` denominator that
            spikes the reward in low-volatility windows.

        What v11 keeps from v10:
          - Drawdown penalty (with λ reduced 5.0 → 1.0). v9's empirical finding
            was 60-72% win rates but most stocks losing money — small wins big
            losses. A mild DD penalty discourages the deep drawdowns we saw
            (-43% to -73%) without overwhelming the primary return signal.
          - Lighter regularization (smaller LSTM, fewer epochs, larger batch,
            higher entropy) — v10 EV dropped 0.99 → 0.80 under these settings,
            evidence that the critic was indeed overfitting. Keep it.

        Other v11 change: clip widened from [-10, 10] to [-20, 20] so single
        ±10% daily moves (e.g. TATAMOTORS bull-market spikes) don't peg the
        reward as they did in v9.
        """
        primary = float(np.clip(log_return * 100.0, -20.0, 20.0))
        ...
        return primary - dd_penalty

    def reset(self, *args, **kwargs):
        # Reset reward-state too — env is reused across rollout episodes in
        # FinRL, and stale equity-peak from a prior episode would contaminate
        # the next.
    ...
    def step(self, action):
        ...
        # v11 REWARD: log-return × 100 (clipped [-20, 20]) minus mild drawdown
        # penalty. See _compute_reward_v11 docstring for the rationale.
        ...
        reward = self._compute_reward_v11(log_return, self.total_asset)
