...
class ValidationCallback(BaseCallback):
    ...
    def __init__(self, val_df, tech_indicators, hmax_value, train_vec_normalize,
        ...
        self.eval_history = []  # list of (timestep, val_return) for diagnostics
    ...
    def _on_step(self):
        ...
        val_return = self._eval_on_val()
        self.eval_history.append((self.num_timesteps, val_return))

        if val_return > self.best_return:
            self.best_return = val_return
            self.model.save(self.save_path)
            if self.verbose:
                print(f"  [val@{self.num_timesteps}] {val_return:+.2f}% — NEW BEST (saved)")
        elif self.verbose:
            print(f"  [val@{self.num_timesteps}] {val_return:+.2f}% "
                  f"(best so far {self.best_return:+.2f}%)")
        return True
    ...
    def _eval_on_val(self):
        ...
        return (final_value / self.initial_amount - 1.0) * 100.0
