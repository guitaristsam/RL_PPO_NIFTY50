...
class IntegerTradingEnv(StockTradingEnv):
    ...
    def __init__(self, **kwargs):
        ...
        # v10 reward state — Moody-Saffell differential Sharpe + drawdown penalty.
        # See _compute_reward_v10 below for derivation.
        self._dsr_eta = 0.01            # EWMA decay; ~100-step horizon
        self._dsr_A = 0.0               # running EWMA of return
        self._dsr_B = 0.0               # running EWMA of return²
        self._equity_peak = float(self.initial_amount)
        self._dd_threshold = 0.10       # only penalize drawdowns deeper than this
        self._dd_lambda    = 5.0        # penalty weight; reward and penalty are
                                        # both ~O(1) so this scales DD vs DSR

    def _compute_reward_v10(self, log_return, total_asset):
        """v10 reward = differential Sharpe ratio - drawdown penalty.

        Differential Sharpe (Moody & Saffell 1998): an online, per-step proxy
        for the Sharpe ratio. Maintains EWMAs A_t (mean return) and B_t (mean
        squared return) and emits

            DSR_t = (B_{t-1} * dA - 0.5 * A_{t-1} * dB) / (B_{t-1} - A_{t-1}²)^(3/2)

        which is the gradient of the Sharpe ratio wrt the new return. This
        gives the agent a Sharpe gradient instead of raw return — small wins
        in low-vol periods get rewarded MORE than equal-size wins in high-vol
        periods, fixing the TATAMOTORS divergence we saw in v9 (its log returns
        in the +338% bull run kept hitting the [-10,10] clip).

        Drawdown penalty: track running peak equity. When drawdown > 10%, add
        a linear penalty proportional to depth. This fixes the v9 small-wins-
        big-losses asymmetry — agents had 60-72% win rates but lost net money.
        """
        eta = self._dsr_eta
        A_prev = self._dsr_A
        B_prev = self._dsr_B
        dA = log_return - A_prev
        dB = (log_return * log_return) - B_prev

        var_proxy = B_prev - A_prev * A_prev
        if var_proxy > 1e-8:
            dsr = (B_prev * dA - 0.5 * A_prev * dB) / (var_proxy ** 1.5)
        else:
            # Cold-start: fall back to log-return until enough history accrues.
            dsr = log_return * 10.0

        # Update EWMAs AFTER computing DSR (uses prev values).
        self._dsr_A = A_prev + eta * dA
        self._dsr_B = B_prev + eta * dB

        # Drawdown penalty
        if total_asset > self._equity_peak:
            self._equity_peak = total_asset
        drawdown = 0.0 if self._equity_peak <= 0 else max(
            0.0, (self._equity_peak - total_asset) / self._equity_peak
        )
        dd_penalty = self._dd_lambda * max(0.0, drawdown - self._dd_threshold)

        return float(dsr - dd_penalty)

    def reset(self, *args, **kwargs):
        # Reset reward-state too — env is reused across rollout episodes in
        # FinRL, and stale EWMA / equity-peak from a prior episode would
        # contaminate the next.
        result = super().reset(*args, **kwargs)
        self._dsr_A = 0.0
        self._dsr_B = 0.0
        self._equity_peak = float(self.initial_amount)
        # Re-round initial positions (super().reset may have set floats)
        self.state[self._shares_slice] = np.round(
            self.state[self._shares_slice]
        ).astype(int)
        return result
    ...
    def step(self, action):
        ...
        # v10 REWARD: differential Sharpe ratio - drawdown penalty.
        # Replaces v9's `log_return * 100, clipped [-10, 10]`. The fixed scale
        # caused TATAMOTORS to peg the clip during +338% bull moves (single
        # 10% days hitting the upper bound) and gave equal weight to small
        # gains in low-vol vs. large gains in high-vol periods.
        if prev_total_asset > 1e-6 and self.total_asset > 1e-6:
            log_return = float(np.log(self.total_asset / prev_total_asset))
        else:
            log_return = -1.0  # bankruptcy penalty
        reward = self._compute_reward_v10(log_return, self.total_asset)
...
def train_ppo_model(train_df, tech_indicators, stock_name, total_timesteps=200000):
    ...
    # v10 hyperparameter tightening to combat the v9 critic overfit
    # (explained_variance ended at 0.95-0.99 across all 7 stocks):
    #   - lstm_hidden_size: 128 -> 64   (less capacity to memorize)
    #   - net_arch: [128] -> [64]       (matched MLP head)
    #   - n_epochs: 5 -> 3              (fewer passes per rollout)
    #   - batch_size: 64 -> 128         (stronger gradient averaging)
    #   - ent_coef: 0.01 -> 0.02        (a bit more exploration to avoid
    #                                    confidently-wrong policies)
    PPO_PARAMS = {
        "learning_rate": 3e-4,
        "n_steps": 512,
        "batch_size": 128,
