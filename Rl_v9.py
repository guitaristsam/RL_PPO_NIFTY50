...
class IntegerTradingEnv(StockTradingEnv):
    ...
    def step(self, action):
        ...
        # v9 REWARD CHANGE: replace FinRL's raw ΔP&L (rupees per day, scaled 1e-3)
        # with daily LOG RETURN. This is scale-invariant, implicitly risk-aware
        # (a -50% drawdown produces a -0.69 reward; the scale of "good" and "bad"
        # days is symmetric), and matches the standard academic formulation for
        # RL trading. We multiply by 100 so typical daily moves (~0.01 log) land
        # in the [-1, 1] range PPO's value head likes.
        ...
        reward = float(np.clip(log_return * 100.0, -10.0, 10.0))
