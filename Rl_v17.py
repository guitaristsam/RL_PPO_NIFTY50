...
class ValidationCallback(BaseCallback):
    ...
    def __init__(self, val_df, tech_indicators, hmax_value, train_vec_normalize,
        ...
        # v17: minimum trades during val for a checkpoint to be eligible.
        # In v16 HDFCBANK, the 50k checkpoint produced 11 trades over the full
        # *test* (~3.5 yrs); during val it was likely even more degenerate
        # (an "agent that does nothing"). Such empty policies pass on flat
        # val periods because holding cash beats any actively-trading attempt.
        # Requiring >= min_val_trades position changes during val rejects
        # those degenerate checkpoints.
        self.min_val_trades = min_val_trades
        self.best_return = -float('inf')
        self.last_eval_step = 0
        self.eval_history = []  # list of (timestep, val_return, val_trades)

    def _on_step(self):
        # _on_step fires once per env step. Trigger eval on cadence.
