# Rl_v55.py — UNRUN single-variable fork of Rl_v18.py.
# CHANGE (one variable): model capacity lstm_hidden_size 128->64 and the
# post-LSTM net_arch [128]->[64]. Everything else is byte-identical to v18
# (106 features kept, reward, callback, splits, all other hyperparameters).
# Rationale: capacity 128->64 is the ONLY single-variable lever positive on BOTH
# independent proxy panels (ITC +12.2pp, TATAMOTORS +9.0pp vs their baselines).
# This promotes that lever to a clean v18 production fork WITHOUT reducing
# features (unlike the invalidated v26). See variants.md "Rl_v55.py".
# Run: `python run_panel.py v55`   (do NOT run frozen files).
import os
...
def train_ppo_model(train_df, tech_indicators, stock_name, total_timesteps=200000,
    ...
    PPO_PARAMS = {
        ...
        "policy_kwargs": {
            "lstm_hidden_size": 64,     # v55: 128->64 capacity cut (#1 reproducible anti-overfit lever)
            "n_lstm_layers": 1,         # 1 layer is plenty; deeper hurts on this size
            "shared_lstm": False,       # separate LSTM for actor & critic
            "enable_critic_lstm": True,
            "net_arch": [64],           # v55: 128->64 to match the reduced LSTM width
