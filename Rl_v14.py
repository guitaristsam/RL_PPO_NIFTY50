...
class IntegerTradingEnv(StockTradingEnv):
    ...
    def __init__(self, **kwargs):
        ...
        # v14 reward state — same deepening-only DD penalty as v13, but with
        # lambda dialed down 20 → 5.
        #
        # v13 RELIANCE regressed badly (+16% vs v12 +85%) despite clean
        # training curves. The deepening-only signal was too strong at lambda=20:
        # a 5% deepening day produced a -1.0 penalty, comparable to a -5%
        # return. That biased the policy away from positions with any chance
        # of short-term volatility — including the steady positions that
        # worked for v12.
        #
        # At lambda=5, a 5% deepening day costs -0.25 (meaningful but not
        # dominant); a 20% crash day costs -1.0.
    ...
    def step(self, action):
        ...
        # v14 REWARD: v9 log_return × 100 (clipped) MINUS a marginal-DD penalty.
        # Penalty fires only when drawdown DEEPENS, lambda=5 (vs v13's 20).
        # See _dd_lambda comment in __init__ for derivation.
