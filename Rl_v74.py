# Rl_v74.py — UNRUN single-variable fork of Rl_v18.py.
# CHANGE (one variable): learning_rate 3e-4 (constant) -> COSINE decay 3e-4 -> 0
# over the full training budget, via SB3's progress_remaining schedule callable.
# (Cosine, not linear: the best-val callback restores the ~100k checkpoint, so
# the win path is a BETTER PEAK, not avoiding late decay; cosine holds LR high
# through the peak region where linear would already be near the rejected 1e-4.)
# Everything else is byte-identical to v18 (106 features, reward, callback,
# splits, n_steps/batch/epochs, policy_kwargs).
# Rationale: constant-LR tweaks (1e-3, 1e-4) were already screened and DISCARDED,
# but a SCHEDULE is a different mechanism — it shrinks the step size in LATE
# training, directly attacking the diagnosed failure mode "val return peaks
# around 100k timesteps then DECAYS with more training." Smaller late steps mean
# less late-training memorization on the region where the val curve degrades.
# LR decay-to-max_steps is the SB3/PPO-recommended default and a classic
# anti-late-overfit lever. See variants.md "Rl_v74.py".
# Run: `python run_panel.py v74`   (do NOT run frozen files).
import os
...
def train_ppo_model(train_df, tech_indicators, stock_name, total_timesteps=200000,
    ...
    # v74: cosine learning-rate decay. SB3 calls the schedule with
    # progress_remaining in [1, 0] (1.0 at the first update, 0.0 at the last).
    # Cosine holds the LR HIGH early (where the best-val checkpoints live, ~100k)
    # and anneals smoothly to 0, unlike linear which drops to ~1e-4 by the peak
    # region (a value already screened+rejected as a constant). yields 3e-4 at
    # start, ->0 at end.
    import math

    def _cosine_lr(progress_remaining):
        # progress_remaining: 1.0 -> 0.0 over training
        return 3e-4 * 0.5 * (1.0 + math.cos(math.pi * (1.0 - progress_remaining)))

    PPO_PARAMS = {
        "learning_rate": _cosine_lr,   # v74: 3e-4 -> 0 cosine decay (was constant 3e-4)
