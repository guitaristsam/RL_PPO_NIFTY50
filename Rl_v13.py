...
class IntegerTradingEnv(StockTradingEnv):
    ...
    def __init__(self, **kwargs):
        ...
        # v13 reward state — v12's DD penalty refined to fire only when the
        # drawdown DEEPENS (not for sitting in a known drawdown).
        #
        # Rationale: v12's penalty fired every step the equity was below peak,
        # so once underwater the agent kept getting punished even if it held
        # the position through a routine pullback. This biased v12 toward
        # exiting positions on small dips → INFY regression (-33pp vs v9) and
        # TATAMOTORS still missing the +338% bull (-19% vs B&H +338%).
        #
        # v13 instead penalizes only the marginal deepening of drawdown:
        #   delta_dd = max(0, dd_t - dd_{t-1})
        #   dd_penalty = lambda * delta_dd
        # Sitting in a 30% DD that doesn't get worse costs nothing. Crashing
        # from 30% to 35% costs lambda * 0.05.
        #
        # Lambda is bumped to 20 because the marginal-deepening signal is much
        # smaller per step than v12's persistent penalty. With lambda=20, a 5%
        # deepening day = 1.0 penalty, comparable to a typical bad return day
        # (log_return * 100 = -5 for a -5% day).
        self._equity_peak = float(self.initial_amount)
        self._prev_drawdown = 0.0
        self._dd_lambda = 20.0
    ...
    def step(self, action):
        ...
        # v13 REWARD: v9 log_return × 100 (clipped) MINUS a marginal-DD penalty.
        # Penalty fires only when drawdown DEEPENS, not while sitting in a
        # known DD. See _dd_lambda comment in __init__ for derivation.
        ...
        dd_penalty = self._dd_lambda * delta_dd
