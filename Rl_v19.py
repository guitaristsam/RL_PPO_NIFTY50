"""
v19: B&H-relative reward.

Hypothesis: v18's reward (log-return * 100, clipped, minus DD penalty) gives the
agent a gradient toward "make money in absolute terms", but says nothing about
beating the buy-and-hold baseline. On strong-trend stocks (RELIANCE +147% B&H,
TATAMOTORS +338% B&H), v18 can technically be making money while still losing
badly to B&H — and PPO's loss has no idea. By subtracting the B&H log-return
from the agent's log-return, we give the policy an *explicit* "alpha" signal:
positive reward only when the agent beats the benchmark for that bar.

The B&H "price" is just the close of the single stock (this is a single-stock
env, so B&H is "buy 1 unit at t and hold"). reward_t = log(eq_t / eq_{t-1}) -
log(close_t / close_{t-1}), then * 100, clipped to [-10, 10]. v12's DD penalty
is preserved so we are still doing a single-variable swap (primary reward only).

Expected effect: should help on RELIANCE / INFY / TATAMOTORS — stocks where v18
underperformed B&H purely because trend strength outpaced the agent's
willingness to stay long. Risk: on flat / negative-trend stocks, the agent gets
positive reward simply by sitting in cash during a benchmark drawdown, which
might encourage degenerate "all cash" policies (cf. ADANIENT in v12). Diagnose
via training-curve clip_fraction (should stay > 0.06 if the gradient is
informative) and final trade count (must exceed min_val_trades to avoid the
ADANIENT degenerate-policy trap).

Why this beats v18 if successful: agent is no longer rewarded for riding the
market — it is rewarded only for outperforming it. That is the actual objective
in real portfolio management work.
"""
...
class IntegerTradingEnv(StockTradingEnv):
    ...
    def step(self, action):
        ...
        # v19: snapshot pre-step B&H "price" — just the close of the single
        # stock — so we can compute the benchmark log-return for this bar
        # and subtract it from the agent's log-return below.
        prev_bh_price = float(prev_prices[0]) if self.stock_dim >= 1 else 1.0
        ...
        # v19 REWARD: B&H-relative log-return × 100 (clipped) MINUS v12 DD penalty.
        # primary = (log(eq_t / eq_{t-1}) - log(bh_t / bh_{t-1})) * 100, clipped.
        # The agent is rewarded ONLY when it outperforms a 1-share-buy-and-hold
        # for that bar. This is the alpha signal v18 is missing.
        if prev_total_asset > 1e-6 and self.total_asset > 1e-6:
            log_return = np.log(self.total_asset / prev_total_asset)
        else:
            log_return = -1.0
        # v19: B&H log-return for the same bar. cur_bh_price = today's close
        # (which is what the env stores in self.state[1] post-step).
        cur_bh_price = float(self.state[1]) if self.stock_dim >= 1 else 1.0
        if prev_bh_price > 1e-6 and cur_bh_price > 1e-6:
            bh_log_return = np.log(cur_bh_price / prev_bh_price)
        else:
            bh_log_return = 0.0
        relative_log_return = log_return - bh_log_return
        primary = float(np.clip(relative_log_return * 100.0, -10.0, 10.0))
