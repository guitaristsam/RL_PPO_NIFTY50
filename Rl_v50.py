...
# ============================================================================
# Rl_v50.py — UNRUN DRAFT (auto/research, 2026-09-04). DO NOT treat as validated.
# Single-variable fork of the v18 PRODUCTION CHAMPION.
#   CHANGE (one variable): observation scaling. v18 fits a single static
#   RobustScaler on the train slice and applies it forever. v50 replaces that
#   with a CAUSAL ROLLING transform (trailing percentile-rank, window W=252)
#   computed on the CONTINUOUS series before the split, so inputs stay bounded
#   and regime-adaptive and early val/test rows legitimately pull trailing PAST
#   rows into their window (no per-split reset). Everything else (features,
#   env, reward, ValidationCallback, hyperparams) is byte-identical to v18.
# Rationale + cautions: see variants.md "Rl_v50.py". UNRUN — not yet validated
# on the panel; must be scored with v37's exposure-adjusted selector + >=20-trade
# gate. Causal by construction (trailing window, current bar excluded), but audit
# with test_indicator_causality.py before trusting.
# ============================================================================
import glob
import time
import random
...
def causal_rolling_rank_normalize(df, feature_cols, window=252):
    """v50: causal, stateless, regime-adaptive normalization of feature columns.

    For each tic and each feature, replace x_t by its trailing-window PERCENTILE
    RANK mapped to [-1, 1], using ONLY the trailing `window` bars STRICTLY BEFORE
    t (the current bar is excluded via shift, so this is causal by construction —
    the value at t cannot change when future rows are appended). Applied on the
    CONTINUOUS per-tic series (before the train/val/test split) so early val/test
    rows pull trailing past rows into their window rather than resetting.

    This REPLACES v18's static train-fit RobustScaler. It is the single variable.
    Distinct from VecNormalize (global running mean/std, not windowed, not rank).
    """
    out = df.copy()
    for tic in out['tic'].unique() if 'tic' in out.columns else [None]:
        mask = (out['tic'] == tic) if tic is not None else out.index == out.index
        sub = out.loc[mask, feature_cols]
        # rolling percentile rank of the last value within each trailing window,
        # excluding the current bar (shift(1)); min_periods=20 lets early bars use
        # an expanding window until `window` history exists.
        def _trailing_rank(s):
            def _rank_last(w):
                # fraction of the trailing window (excl. current) strictly below
                # the current value; current value is the last element of w.
                cur = w[-1]
                past = w[:-1]
                if len(past) == 0:
                    return 0.5
                return float((past < cur).sum()) / float(len(past))
            # window+1 so each window carries the current bar plus `window` trailing
            r = s.rolling(window=window + 1, min_periods=2).apply(_rank_last, raw=True)
            return r
        ranked = sub.apply(_trailing_rank, axis=0)
        # map [0,1] -> [-1,1]; leading NaN (no history) -> 0 (neutral, matches
        # v18's median=0 convention for unfillable indicators).
        ranked = (ranked * 2.0 - 1.0).fillna(0.0)
        out.loc[mask, feature_cols] = ranked.values
    return out
...
def train_ppo_model(train_df, tech_indicators, stock_name, total_timesteps=200000,
    ...
    try:
        ...
        if val_df is not None:
            best_path = os.path.join(TRAINED_MODEL_DIR, f"{stock_name}_best.zip")
...
def process_stock(file_path):
    ...
    try:
        ...
        # v50: apply the CAUSAL ROLLING RANK normalization on the CONTINUOUS frame
        # (before the split) so early val/test rows pull trailing PAST rows into
        # their window — no per-split reset. This is the single variable vs v18 and
        # REPLACES the static train-fit RobustScaler (per-split calls below use
        # skip_scaling=True). Causal by construction: current bar excluded.
        processed_raw = causal_rolling_rank_normalize(processed_raw, tech_indicators, window=252)
        ...
        # v50: indicators are ALREADY causally rank-normalized on the continuous
        # frame above, so skip the static RobustScaler on every split (single
        # variable vs v18). Format/NaN handling still runs. VecNormalize in
        # train_ppo_model is kept ON for a clean single-variable diff (it is ~a
        # no-op on already-bounded inputs; note the interaction in the readout).
        train_df, _, scalers = prepare_data_for_finrl(train_raw, skip_scaling=True)
        val_df,   _, _       = prepare_data_for_finrl(val_raw,   skip_scaling=True)
        test_df,  _, _       = prepare_data_for_finrl(test_raw,  skip_scaling=True)
        ...
        model, vecnorm, hmax_value = train_ppo_model(
