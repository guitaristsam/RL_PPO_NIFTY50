# ===========================================================================
# Rl_v31.py — UNRUN DESIGN DRAFT (auto/research 2026-09-02). DO NOT treat as
# a validated baseline. Single-variable fork of the v26 CHAMPION (22 curated
# indicators): the ONLY change vs Rl_v26.py is the PPO learning_rate — a
# constant 3e-4 becomes a LINEAR decay 3e-4 -> 0 over training. Rationale:
# val return peaks ~100k then decays (late-training overfit); shrinking the
# step size late freezes the policy near its best-generalizing point. This
# is distinct from the already-screened CONSTANT-lr changes (1e-4, 1e-3,
# both regressed) — a schedule, not a level. See variants.md "Rl_v31.py".
# ===========================================================================
import os
...
# v26: FEATURE REDUCTION. Single-variable change vs v18: cut the indicator
# list from 98 names to 22 curated, well-understood ones. Evidence: explained
# variance pegs at 0.95-0.99 while test returns lag (textbook critic overfit),
# and 98 indicators x 1 stock x ~2,500 train bars is data-starved for a
# 101-dim observation. Hypothesis: a 25-dim observation generalises better.
# All 22 names are a subset of v18's audited-causal list.
list_of_indicators = [
    # trend / direction
...
def train_ppo_model(train_df, tech_indicators, stock_name, total_timesteps=200000,
    ...
    PPO_PARAMS = {
        # v31: linear decay 3e-4 -> 0 (SB3 passes progress_remaining 1->0).
