# Rl_v69.py — UNRUN DRAFT (2026-09-06, auto-research).
# Fork of Rl_v18.py. SINGLE VARIABLE vs v18: asymmetric critic-only capacity —
# net_arch [128] -> {"pi":[128], "vf":[64]} (actor width unchanged, value head halved).
# Motivation: the diagnostics pin the CRITIC as the overfitter (train EV 0.95-0.99,
# poor test); v55-v57 shrank both heads symmetrically. See variants.md "Rl_v69.py".
# NOT RUN and NOT torch/sb3-smoke-tested here (no torch in this env); ast-parse clean.
# A run-routine should confirm SB3 accepts the dict net_arch for MlpLstmPolicy and read
# whether critic EV falls below 0.99 without the actor collapsing to cash (>=20 trades).
import os
...
def train_ppo_model(train_df, tech_indicators, stock_name, total_timesteps=200000,
    ...
    PPO_PARAMS = {
        ...
        "policy_kwargs": {
            ...
            # v69 SINGLE CHANGE vs v18: asymmetric head capacity. The diagnostics
            # pin the CRITIC as the overfitter (train EV 0.95-0.99, poor test); v55-v57
            # shrank both heads symmetrically. Here the actor head keeps width 128 while
            # the value (critic) head is halved to 64, so the critic has less capacity to
            # memorize per-episode training returns while the actor stays expressive.
            # SB3 dict net_arch form; LSTM size unchanged (changing it too = 2 variables).
            "net_arch": {"pi": [128], "vf": [64]},   # v18 was: [128]
