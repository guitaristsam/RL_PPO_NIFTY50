...
class IntegerTradingEnv(StockTradingEnv):
    def __init__(self, **kwargs):
        ...
        # v6 logs spammed "Near-zero price detected" every rollout. Print once per env.
...
def train_ppo_model(train_df, tech_indicators, stock_name, total_timesteps=200000):
    ...
    PPO_PARAMS = {
        ...
        "n_steps": 1024,          # v6 used 2048 ≈ 8 yrs of daily data per rollout — bigger
                                  # than most stocks' train sets. v6 curves showed stuck std
                                  # (0.97) and oscillating value_loss; halving n_steps gives
                                  # 2× more gradient updates per 200k timesteps.
...
def test_ppo_model(trained_model, test_df, tech_indicators, stock_name,
    ...
    while not done and step_count < max_steps:
        try:
            ...
            current_positions = {}
            if hasattr(underlying_env, 'state') and len(underlying_env.state) > 1:
