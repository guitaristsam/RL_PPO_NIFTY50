...
# ============================================================================
# Rl_v37.py — UNRUN DRAFT (auto-research, 2026-09-03). Do NOT treat as validated.
#
# SINGLE CHANGE vs Rl_v18.py (the production champion): the ValidationCallback
# selection objective. v18 saved the best-by-raw-val-RETURN checkpoint, which in
# a bear val window is a do-nothing CASH-HOLD (the confirmed v26 / proxy-champion
# artifact). v37 instead selects on the EXPOSURE-ADJUSTED active-return info
# ratio (exposure-matched Jensen's alpha) with a DUAL activity gate
# (>=20 trades AND <= val-bar-count). A cash-hold has exposure ~ 0, so its
# active return ~ 0 and its score ~ 0 -> it can no longer win the checkpoint
# race. Env, reward, features, hyperparameters, warmup(100k) all identical to
# v18. See variants.md "Rl_v37.py" for the full rationale and advisor note.
# Measurement-integrity PREREQUISITE, not an alpha source: a correct selector
# stops the loop being fooled; it cannot manufacture edge. NOT YET RUN.
# ============================================================================
import glob
import time
import random
...
class ValidationCallback(BaseCallback):
    ...
    def __init__(self, val_df, tech_indicators, hmax_value, train_vec_normalize,
                 ...
                 min_val_trades=20, max_val_trades=None, warmup_steps=100000,
                 verbose=1):
        ...
        # v37: DUAL activity gate. Floor (>=20 trades) enforces the frontier
        # "genuine active policy" bar and rejects cash-holds; ceiling rejects a
        # policy that trades essentially every bar (cost-bleed churn — the
        ...
        # v37: select on the EXPOSURE-ADJUSTED active-return info ratio, not raw
        # return (v18) or raw Sharpe (v20). See _eval_on_val for why raw metrics
        # reward a bear-window cash-hold and this one does not.
        self.best_score = -float('inf')
        self.best_return = -float('inf')  # kept for the restore-time print only
    ...
    def _on_step(self):
        ...
        val_return, val_score, val_trades, val_mean_exp = self._eval_on_val()
        self.eval_history.append(
            (self.num_timesteps, val_return, val_score, val_trades, val_mean_exp))

        # v37: DUAL activity gate — floor (reject cash-holds) AND ceiling
        # (reject every-bar churn). max defaults to the val bar count.
        ...
        if eligible and val_score > self.best_score:
            ...
            print(f"  [val@{self.num_timesteps}] adj_IR={val_score:+.3f} "
                  f"ret={val_return:+.2f}% exp={val_mean_exp:.2f} "
                  f"(trades={val_trades}) (best adj_IR {self.best_score:+.3f})")
        return True
    ...
    def _eval_on_val(self):
        ...
        sd = underlying.stock_dim
        # Position-delta trade counter. underlying state layout:
        #   state[1+stock_dim : 1+2*stock_dim] = shares per stock.
        prev_shares = np.array(
            underlying.state[1 + sd : 1 + 2 * sd], dtype=np.int64
        ).copy()
        trade_count = 0
        done = False
        step_count = 0

        # v37: record per-step portfolio value, invested exposure, and price so we
        # can score the EXPOSURE-ADJUSTED active return. Seed with the pre-step
        # state so diffs line up with the price series.
        def _snapshot():
            price = float(underlying.state[1])  # single-stock env: first price slot
            shares = float(underlying.state[1 + sd])
            total = float(getattr(underlying, 'total_asset',
                                  float(underlying.state[0]) + shares * price))
            exp = (shares * price / total) if total > 1e-9 else 0.0
            return total, price, exp
        t0, p0, e0 = _snapshot()
        pv_list, price_list, exp_list = [t0], [p0], [e0]
        ...
        while not done and step_count < max_steps:
            ...
            tt, pp, ee = _snapshot()
            pv_list.append(tt); price_list.append(pp); exp_list.append(ee)
            step_count += 1
        ...
        # v37 SCORE: exposure-matched Jensen's alpha, annualized info ratio.
        #   active_t = pv_ret_t - exposure_{t-1} * bh_ret_t
        # A cash-hold has exposure ~ 0, so active_t ~ pv_ret_t ~ 0 in a bear
        # window -> score ~ 0 (it can no longer masquerade as a val winner).
        # Raw return (v18), raw Sharpe (v20) and naive IR-over-B&H all fail this
        # because a low-vol cash-hold vs a FALLING B&H shows a high ratio.
        pv = np.asarray(pv_list, dtype=np.float64)
        pr = np.asarray(price_list, dtype=np.float64)
        ex = np.asarray(exp_list, dtype=np.float64)
        val_mean_exp = float(np.mean(ex)) if ex.size else 0.0
        if pv.size >= 3 and np.all(pv[:-1] > 1e-9) and np.all(pr[:-1] > 1e-9):
            pv_ret = np.diff(pv) / pv[:-1]
            bh_ret = np.diff(pr) / pr[:-1]
            active = pv_ret - ex[:-1] * bh_ret          # exposure-adjusted
            sd_active = float(np.std(active))
            if sd_active > 1e-9:
                score = float(np.mean(active) / sd_active) * np.sqrt(252.0)
            else:
                score = 0.0
        else:
            score = -float('inf')  # too short to score -> ineligible
        return val_return_pct, score, trade_count, val_mean_exp
    ...
    try:
        ...
        if val_df is not None:
            ...
            val_cb = ValidationCallback(
                ...
                min_val_trades=20,   # v37: frontier genuine-active bar
                verbose=1,
            )
        ...
        if best_path is not None and os.path.exists(best_path):
            ...
            print(f"  Restoring best-val model (adj_IR={best_score:+.3f}, "
                  f"val_return={best_return:+.2f}%) from {best_path}")
