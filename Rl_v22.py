"""
v22: ensemble-friendly seeds.

Hypothesis: v9-batch and v18-batch results show high cross-stock variance even
with a fixed seed. RecurrentPPO at 200k timesteps is in a regime where the
optimization landscape has many shallow minima — different seeds find
different policies. A 3-seed ensemble that AVERAGES the continuous actions
before discretizing should cut policy-level variance roughly by 1/√3 (assuming
uncorrelated seed errors), without changing the underlying algorithm.

Single-variable change vs v18: NO algorithmic change. Just a `seed_offset`
plumbed through `process_stock` and `train_ppo_model` so the same code can be
called 3 times per stock with different seeds, producing 3 independent
checkpoints. Saved model path includes the seed: `{stock}_seed{N}_ppo.zip` (and
`_seed{N}_vecnorm.pkl`, `_seed{N}_best.zip`). The `consolidated_report.txt` and
per-stock report flow remains identical for a single-seed run; the ensemble
helper `ensemble_predict.py` (separate file, ~30 lines) loads N saved models
and averages their continuous actions before passing to one shared env.

Expected effect: lower cross-seed variance on the worst stocks (TATAMOTORS,
INFY) where v18 currently swings ±20pp depending on random init. Diagnose by
running v22 with 3 seeds on RELIANCE, INFY, TATAMOTORS and comparing the
per-seed test return to the ensemble test return.

Why this beats v18 if successful: same training cost × N, but variance in the
deployed policy drops, which is the harder problem at this point in the
project (v18's training is converging — the mean is decent, the variance is the
issue).
"""
...
class ValidationCallback(BaseCallback):
    """
    v16: periodic deterministic evaluation on the validation slice with
    best-checkpoint saving.

    Why this exists: v15 (1M timesteps) showed direct evidence of overfit —
    `std` halved repeatedly, EV pegged at 0.99 on training rollouts, but
    test return *dropped* (-11pp vs v12 at 200k). Without a validation
    signal we have no principled way to pick the right stopping point per
    stock; some need 200k, others might need 50k or 500k.

    On `_on_step` cadence: we trigger every `eval_freq` env steps. The
    eval rebuilds a fresh val env wrapped in VecNormalize, copies running
    stats from the train wrapper (frozen), runs the deterministic policy
    through the val period, and records the final portfolio return. Best
    checkpoint is saved to `save_path`; caller restores it after .learn().
    """
    ...
    def __init__(self, val_df, tech_indicators, hmax_value, train_vec_normalize,
        ...
        # was not converged. Once locked in as best, every later (more-trained)
    ...
    def _on_step(self):
        ...
        # v18: skip evals before warmup_steps to avoid locking in under-trained policies.
    ...
    def _eval_on_val(self):
        # Build a fresh val env wrapped in VecNormalize. Sync running stats from
        # the live train wrapper so the agent sees observations on the same
        # distribution it was trained on. Freeze stats during eval.
        val_env_vec = DummyVecEnv([lambda: create_trading_environment(
            self.val_df, self.tech_indicators,
            initial_amount=self.initial_amount, hmax=self.hmax_value
        )])
        val_vn = VecNormalize(val_env_vec, norm_obs=True, norm_reward=False,
                              clip_obs=10.0, gamma=0.99)
        val_vn.obs_rms = copy.deepcopy(self.train_vn.obs_rms)
        val_vn.ret_rms = copy.deepcopy(self.train_vn.ret_rms)
        ...
        max_steps = len(sorted(self.val_df['date'].unique())) - 1
        # Position-delta trade counter. underlying state layout:
        #   state[1+stock_dim : 1+2*stock_dim] = shares per stock.
...
def train_ppo_model(train_df, tech_indicators, stock_name, total_timesteps=200000,
                    val_df=None, seed_offset=0):
    """
    Train PPO model using FinRL with TensorBoard logging.

    v22: `seed_offset` shifts the global seed (default base 42) so a single
    stock can be trained N times with different seeds for ensembling. The
    saved model and VecNormalize files include the effective seed in the
    filename so multiple seeds coexist on disk.
    """
    ...
    # v22: effective seed = 42 + seed_offset. seed_offset=0 reproduces v18.
    effective_seed = 42 + int(seed_offset)
    PPO_PARAMS = {
        "learning_rate": 3e-4,
    ...
    try:
        ...
        if val_df is not None:
            # v22: seed-suffixed best-checkpoint filename so multi-seed runs
            # don't overwrite each other.
            best_path = os.path.join(
                TRAINED_MODEL_DIR, f"{stock_name}_seed{effective_seed}_best.zip"
            )
        ...
        # Restore best-val checkpoint if validation was enabled.
        # In v17, ALL eval points may be ineligible (every checkpoint produced
        ...
        if best_path is not None and os.path.exists(best_path):
            ...
            print(f"  WARNING: no eligible val checkpoint was saved "
                  f"(all evals had < {callbacks[0].min_val_trades} trades). "
                  f"Using final-iteration model instead.")

        print(f"  Training for {stock_name} completed successfully!")
        # v22: also return effective_seed so caller can compose seed-specific
        # output filenames for the ensemble.
        return model_ppo, env_train, hmax_value, effective_seed
...
def process_stock(file_path, seed_offset=0):
    """
    Process a single stock from file loading to reporting.

    v22: `seed_offset` (default 0) shifts the base seed of 42. To train an
    ensemble, call `process_stock(path, seed_offset=k)` for k=0,1,2,…. Each
    call produces a distinct `{stock}_seed{N}_ppo.zip` and matching
    VecNormalize stats; the per-stock report is suffixed with the seed so
    multiple seeds don't clobber each other's reports.
    """
    ...
    print(f"\n{'='*50}")
    print(f"PROCESSING {stock_name} (seed_offset={seed_offset})")
    print(f"{'='*50}")

    # v22: seed-aware resume guard. Each seed gets its own report file.
    effective_seed_preview = 42 + int(seed_offset)
    report_suffix = f"_seed{effective_seed_preview}" if seed_offset else ""
    existing_report = os.path.join(
        stock_result_dir, f"{stock_name}{report_suffix}_report.txt"
    )
    if os.path.exists(existing_report):
        print(f"  Skipping {stock_name} (seed_offset={seed_offset}) — "
              f"report already exists at {existing_report}")
        return _parse_existing_report(existing_report, stock_name)

    # Per-stock global seeding — v22 uses the effective seed (42 + offset)
    # so each ensemble member is independently reproducible.
    ...
    try:
        ...
        # Step 5: Split data 70/15/15 (train/val/test) per stock timeline (v16).
        # v6-v15 used 80/20 train/test only. v16 carves out a chronological val
        # slice for early stopping; see ValidationCallback for rationale.
        ...
        # Fit scaler on training data only, then apply same scaler to val and test.
        ...
        print(f"  Split: train < {train_split_date} <= val < {val_split_date} <= test")
        print(f"    Training:   {len(train_df)} rows ({len(train_df['date'].unique())} dates)")
        print(f"    Validation: {len(val_df)} rows ({len(val_df['date'].unique())} dates)")
        print(f"    Testing:    {len(test_df)} rows ({len(test_df['date'].unique())} dates)")

        # Step 6: Train PPO model with val-based early stopping
        model, vecnorm, hmax_value, effective_seed = train_ppo_model(
            train_df, tech_indicators, stock_name, val_df=val_df,
            seed_offset=seed_offset,
        )

        # v22: seed-suffixed model and VecNormalize paths so multiple ensemble
        # members can coexist on disk for the same stock.
        seed_suffix = f"_seed{effective_seed}"
        model_path = os.path.join(TRAINED_MODEL_DIR, f"{stock_name}{seed_suffix}_ppo.zip")
        model.save(model_path)
        vecnorm_path = os.path.join(TRAINED_MODEL_DIR, f"{stock_name}{seed_suffix}_vecnorm.pkl")
        ...
        # Step 8: Generate report. v22: include the seed suffix in the report
        # filename for non-default seeds (seed_offset>0) so multiple ensemble
        # members produce distinct reports under the same stock directory.
        report_stock_name = stock_name + (seed_suffix if seed_offset else "")
        ppo_metrics, buy_hold_metrics, trade_summary = create_comprehensive_report(
            df_account_value, trade_logger, test_df, report_stock_name, stock_result_dir
        )

        # Save outputs (also seed-suffixed for non-default seeds).
        acct_name = "account_value" + (seed_suffix if seed_offset else "")
        trades_name = "trades" + (seed_suffix if seed_offset else "")
        actions_name = "actions" + (seed_suffix if seed_offset else "")
        df_account_value.to_csv(
            os.path.join(stock_result_dir, f"{acct_name}.csv"), index=False
        )
        df_actions.to_csv(
            os.path.join(stock_result_dir, f"{actions_name}.csv"), index=False
        )
        trade_logbook = trade_logger.get_trade_logbook()
        if not trade_logbook.empty:
            trade_logbook.to_csv(
                os.path.join(stock_result_dir, f"{trades_name}.csv"), index=False
            )
