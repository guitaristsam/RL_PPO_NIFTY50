...
def train_ppo_model(train_df, tech_indicators, stock_name, total_timesteps=1000000):
    """
    Train RecurrentPPO model using FinRL with TensorBoard logging.

    v15: 5x longer training (200k → 1M). v12 training curves ended with EV
    at 0.95-0.99 (critic overfit on training rollouts) but PPO still
    underperformed B&H on test. RecurrentPPO trading systems in literature
    typically converge at 1M–5M steps. This is the "more compute" test
    before reaching for structural changes (validation split, target-
    exposure action). Wall time per stock at v12 fps (~280): ~60 min.
    """
